            ...
            if features is not None:
                ...
                if img_indices is not None and len(img_indices) > 0:
                    ...
                    with col1:
                        st.header("I")
                        try:
                            st.image(os.path.normpath(img_files_list[img_indices[0][0]]))
                        except Exception as e:
                            st.error(f"Error opening '{img_files_list[img_indices[0][0]]}': {e}")

                    with col2:
                        st.header("II")
                        try:
                            st.image(os.path.normpath(img_files_list[img_indices[0][1]]))
                        except Exception as e:
                            st.error(f"Error opening '{img_files_list[img_indices[0][1]]}': {e}")

                    with col3:
                        st.header("III")
                        try:
                            st.image(os.path.normpath(img_files_list[img_indices[0][2]]))
                        except Exception as e:
                            st.error(f"Error opening '{img_files_list[img_indices[0][2]]}': {e}")

                    with col4:
                        st.header("IV")
                        try:
                            st.image(os.path.normpath(img_files_list[img_indices[0][3]]))
                        except Exception as e:
                            st.error(f"Error opening '{img_files_list[img_indices[0][3]]}': {e}")

                    with col5:
                        st.header("V")
                        try:
                            st.image(os.path.normpath(img_files_list[img_indices[0][4]]))
                        except Exception as e:
                            st.error(f"Error opening '{img_files_list[img_indices[0][4]]}': {e}")
                else:
