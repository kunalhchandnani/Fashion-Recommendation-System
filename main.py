                ...
                if img_indices is not None and len(img_indices) > 0:
                    ...
                    for i, col in enumerate(columns):
                        with col:
                            st.header(f"{i+1}")
                            try:
                                image_path = os.path.normpath(img_files_list[img_indices[0][i]])
                                if os.path.exists(image_path):
                                    st.image(image_path)
                                else:
                                    st.error(f"Error opening '{image_path}': File does not exist.")
                            except Exception as e:
                                st.error(f"Error opening '{img_files_list[img_indices[0][i]]}': {e}")
                else:
